-- Gespack schema
SET search_path TO gespack,public;
...
-- 2) Normalizar email a minúsculas/trim en users
...
CREATE TABLE user_dashboard_config (
  ...
  -- 👈 SIN FK a sites para permitir site_id = 0
);

-- Añadir comentario DESPUÉS de crear la tabla (sintaxis PostgreSQL)
COMMENT ON COLUMN user_dashboard_config.site_id IS '0 = configuración global para todos los sitios';
...
CREATE TABLE action_priority_types (
    ...
    description TEXT,                                                   -- Descripción del tipo de prioridad
    is_active BOOLEAN DEFAULT TRUE,                                             -- ACTIVO
    created_by TEXT,                                                        -- CREADOR
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,                         -- FECHA_CREACION
    updated_by TEXT,                                                       -- MODIFICADOR
...
CREATE TABLE order_payments (
    ...
    -- Relación opcional con catálogo de tipos de pago
...
DROP INDEX IF EXISTS idx_order_payments_payment_type_id CASCADE;
...
-- Tipo ENUM de estado (como tenías)
...
CREATE TABLE orders (
  ...
  -- 🔒 Bloquear borrado de usuarios con histórico
...
-- Índices (como tenías + algunos útiles)
...
VALUES ('Iván', crypt('P@rcel2025', gen_salt('bf')), 'ilopez@parcelontime.es', TRUE, TRUE, 1)
ON CONFLICT (email) DO NOTHING;

-- Vinculaciones básicas (idempotentes)
...
BEGIN
  FOR v_idx IN 1..array_length(v_emails,1) LOOP
    INSERT INTO users (user_name, user_password, email, is_admin, is_active, created_by)
    VALUES ('TEST '||v_idx, crypt('P@rcel2025', gen_salt('bf')), v_emails[v_idx], FALSE, FALSE, 1) -- 👈 BCRYPT
    ON CONFLICT (email) DO NOTHING;
...
-- =========================================================
-- SEED PRINCIPAL (idempotente) – order_sources, customer_types, brands x3
-- =========================================================
DO $$
DECLARE
  ...
  arr_first_names text[] := ARRAY['Lucía','Mateo','Sofía','Hugo','Valeria','Daniel','Paula','Leo','Martina','Pablo'];
  arr_last_names  text[] := ARRAY['García','Martínez','López','Sánchez','Fernández','Gómez','Díaz','Ruiz','Hernández','Navarro'];
  v_fn text; v_ln text; idx int;

  -- Marca a usar por pedido (según acción)
...
BEGIN
  FOREACH s IN ARRAY site_ids LOOP
    ...
    IF v_category_id IS NULL THEN
      ...
      VALUES (s, 'CategoriaTest', 'Categoría de prueba', v_created_by)
    ...
    IF NOT FOUND THEN
      INSERT INTO order_sources (site_id, source_name, description, is_active, created_by)
      VALUES (s, 'Phone', 'Pedido por teléfono', TRUE, v_created_by);
    END IF;
    ...
    IF NOT FOUND THEN
      ...
      VALUES (s, 1, 'Retail', 'Cliente minorista estándar', v_created_by);
    END IF;
    ...
    FROM actions a
    WHERE a.site_id = s AND a.action_name = 'Campaña Primavera';
    IF v_action_prim IS NULL THEN
      ...
      VALUES (s, 'Campaña Primavera', 'Promoción Primavera', NOW(), v_brand1_id, NOW(), v_category_id, TRUE, v_created_by)
      RETURNING action_id INTO v_action_prim;
    END IF;

    SELECT a.action_id INTO v_action_ver
    FROM actions a
    WHERE a.site_id = s AND a.action_name = 'Campaña Verano';
    IF v_action_ver IS NULL THEN
      INSERT INTO actions (
        site_id, action_name, description, launch_date, brand_id, deposit_date,
        action_category_id, is_active, created_by
      )
      VALUES (s, 'Campaña Verano', 'Promoción Verano', NOW(), v_brand2_id, NOW(), v_category_id, TRUE, v_created_by)
    ...
    FOR j IN 1..5 LOOP
      ...
      IF v_customer_id IS NULL THEN
        ...
        VALUES (
          ...
          (ARRAY['Centro','Salamanca','Retiro','Usera','Latina','Chamartín'])[(j % 6) + 1],
          '2800' || j, 'Madrid', '60012345' || j,
          lower(translate(v_fn,'áéíóúÁÉÍÓÚ','aeiouAEIOU')) || '.' || lower(v_ln) || '+site' || s || '@example.com',
          true, v_created_by
        )
      ...
      --    - Sólo para una parte de los clientes (random() < 0.35)
      --    - Asigna un tipo aleatorio de customer_marked_types por site
      --    - Respeta la unicidad (site_id, name) generando name único
      --------------------------------------------------------------------
      ...
      SELECT
        ...
        (mt.name || ' - ' || c.customer_code) AS name,   -- único por site
      ...
      -- limitar un número razonable por site en seeds iniciales
      LIMIT 150;
      ...
      ----------------------------------------------------------------
      -- 3) PEDIDOS – con order_source_id (usamos Web)
      --    Marca del pedido según acción:
...
BEGIN
  ...
  LOOP
    ...
    VALUES (
      ...
      COALESCE(r.baddr1, 'Dirección ' || r.site_id), r.baddr2, r.baddr3, r.baddr4, r.bcp, r.bcity, r.bphone,
      COALESCE(r.sfn || ' ' || r.sln, r.ofn || ' ' || r.oln, 'Cliente Seed'),
      COALESCE(r.saddr1, COALESCE(r.baddr1, 'Dirección ' || r.site_id)), r.saddr2, r.saddr3, r.saddr4, COALESCE(r.scp, r.bcp), COALESCE(r.scity, r.bcity), COALESCE(r.sphone, r.bphone),
      v_created_by
    );
  END LOOP;
END$$;
...
-- ============================================
-- Todos usan la misma función update_timestamp() y el mismo nombre de trigger
