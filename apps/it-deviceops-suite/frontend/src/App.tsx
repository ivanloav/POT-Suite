...
import SuiteLogin from './pages/SuiteLogin';
...
function App() {
  ...
  useEffect(() => {
    ...
    const bootstrapAuth = async () => {
      const isAuthRoute =
        window.location.pathname.startsWith('/login') || window.location.pathname.startsWith('/sso');

      if (!isAuthenticated && !token && isAuthRoute) {
        setAuthChecked(true);
        return;
      }
  ...
  return (
    <QueryClientProvider client={queryClient}>
      <BrowserRouter>
        <Routes>
          <Route path="/sso" element={<SuiteLogin />} />
