...
import AssetCpuSegmentForm from '@/components/forms/catalogs/AssetCpuSegmentForm';
import toast from 'react-hot-toast';
import { ActionButton } from '@/components/shared/ActionButton';

export default function AssetCpuSegmentsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['asset-cpu-segments', selectedSiteId, filters],
    queryFn: () => assetCpuSegmentService.getAll(),
  });

  const handleRefresh = () => {
    queryClient.invalidateQueries({ queryKey: ['asset-cpu-segments'] });
  };
  ...
  const handleExport = async () => {
    try {
      const blob = await assetCpuSegmentService.exportToExcel();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'segmentos-cpu.xlsx';
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
      toast.success('Segmentos exportados exitosamente');
    } catch (error: any) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const blob = await assetCpuSegmentService.downloadTemplate();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'asset-cpu-segments-template.xlsx';
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await assetCpuSegmentService.importFromExcel(file);
      ...
      if (result.data?.duplicados && result.data.duplicados.length > 0) {
        ...
        if (result.data.insertados > 0) {
          toast.success(`${result.data.insertados} segmentos creados. Se detectaron ${result.data.duplicados.length} duplicados.`);
        } else {
          toast(`Se detectaron ${result.data.duplicados.length} segmentos duplicados. Revisa el modal para decidir si actualizar.`);
        }
        ...
        toast.success(`${result.data.insertados} segmentos creados exitosamente`);
      }
  ...
  const handleUpdateDuplicates = async () => {
    ...
    try {
      const result = await assetCpuSegmentService.updateDuplicatesFromExcel(duplicatesData);
      toast.success(result.message || `${result.data.actualizados} segmentos actualizados`);
  ...
  const columns: Column<any>[] = [
    ...
    },
    {
      ...
      render: (segment) => (
        <button
          ...
          title="Ver detalles del segmento"
        >
          {segment.name || '-'}
        </button>
      ),
    ...
    },
    {
      ...
      width: '12%',
      sortable: true,
      render: (segment) => (segment.creator?.userName || '-'),
    },
    {
      key: 'createdAt',
      label: 'Creación',
      sortable: true,
      render: (segment) => (segment.createdAt ? new Date(segment.createdAt).toLocaleString('es-ES') : '-'),
    },
  ];
  ...
  return (
    <div>
      ...
      <div className="card">
        {isLoading ? (
          <div className="text-center py-12">
            <p className="text-gray-600 dark:text-gray-400">Cargando segmentos de CPU...</p>
          </div>
        ) : (
          <DataTable
            data={data?.data || []}
            ...
            emptyMessage="No se encontraron segmentos de CPU"
            defaultPageSize={10}
          />
        )}
      </div>
      ...
      <Modal
        ...
        title="Crear Nuevo Segmento de CPU"
        size="lg"
      >
        <AssetCpuSegmentForm
          mode="create"
      ...
      {/* Modal Detalles Segmento */}
      <Modal
        isOpen={!!selectedSegmentId}
        onClose={handleDetailsClose}
        title="Detalles del Segmento de CPU"
        size="lg"
      ...
      >
        <AssetCpuSegmentForm
          ...
          onSuccess={() => queryClient.invalidateQueries({ queryKey: ['asset-cpu-segments'] })}
      ...
      <DuplicatesModal
        ...
        title="Duplicados de Segmentos de CPU"
        message="Se encontraron segmentos duplicados. ¿Deseas actualizarlos?"
      />
    </div>
  );
}
