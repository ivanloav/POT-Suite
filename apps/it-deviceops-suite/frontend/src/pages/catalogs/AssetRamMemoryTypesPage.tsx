...
import AssetRamMemoryTypeForm from '@/components/forms/catalogs/AssetRamMemoryTypeForm';
import toast from 'react-hot-toast';
import { ActionButton } from '@/components/shared/ActionButton';

export default function AssetRamMemoryTypesPage() {
  ...
  const [selectedMemoryTypeId, setSelectedMemoryTypeId] = useState<number | null>(null);
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['asset-ram-memory-types', selectedSiteId, filters],
    queryFn: () => assetRamMemoryTypeService.getAll(),
  });

  const handleRefresh = () => {
    queryClient.invalidateQueries({ queryKey: ['asset-ram-memory-types'] });
  };
  ...
  const handleMemoryTypeClick = (memoryTypeId: number) => {
    setSelectedMemoryTypeId(memoryTypeId);
  };
  ...
  const handleExport = async () => {
    try {
      const blob = await assetRamMemoryTypeService.exportToExcel();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'tipos-memoria-ram.xlsx';
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
      toast.success('Tipos de memoria exportados exitosamente');
    } catch (error: any) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const blob = await assetRamMemoryTypeService.downloadTemplate();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'asset-ram-memory-types-template.xlsx';
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await assetRamMemoryTypeService.importFromExcel(file);
  ...
  const handleUpdateDuplicates = async () => {
    ...
    try {
      const result = await assetRamMemoryTypeService.updateDuplicatesFromExcel(duplicatesData);
  ...
  const columns: Column<any>[] = [
    ...
    },
    {
      ...
      render: (type) => (
        <button
          onClick={() => handleMemoryTypeClick(type.id)}
          ...
          title="Ver detalles del tipo de memoria"
        >
          {type.name || '-'}
        </button>
      ),
  ...
  return (
    <div>
      ...
      <div className="card">
        {isLoading ? (
          <div className="text-center py-12">
            <p className="text-gray-600 dark:text-gray-400">Cargando tipos de memoria RAM...</p>
          </div>
        ) : (
          <DataTable
            data={data?.data || []}
            ...
            emptyMessage="No se encontraron tipos de memoria RAM"
            defaultPageSize={10}
          />
        )}
      </div>
      ...
      <Modal
        ...
        title="Crear Nuevo Tipo de Memoria RAM"
        size="lg"
      >
        <AssetRamMemoryTypeForm
          mode="create"
      ...
      <Modal
        isOpen={!!selectedMemoryTypeId}
        onClose={handleDetailsClose}
        title="Detalles del Tipo de Memoria RAM"
        size="lg"
        headerActions={modalHeaderButtons}
      >
        <AssetRamMemoryTypeForm
          mode="edit"
          memoryTypeId={selectedMemoryTypeId!}
          onSuccess={() => queryClient.invalidateQueries({ queryKey: ['asset-ram-memory-types'] })}
      ...
      <DuplicatesModal
        ...
        title="Duplicados de Tipos de Memoria RAM"
        message="Se encontraron tipos duplicados. ¿Deseas actualizarlos?"
      />
    </div>
  );
}
