...
import AssetModelForm from '@/components/forms/catalogs/AssetModelForm';
...
export default function AssetModelsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['asset-models', selectedSiteId, filters],
    queryFn: () => {
      const isActive = filters.isActive === 'true' ? true : filters.isActive === 'false' ? false : undefined;
      return AssetModelService.getAll({ isActive, siteId: selectedSiteId || undefined });
    },
  });
  ...
  const handleExport = async () => {
    try {
      const blob = await AssetModelService.exportToExcel();
      ...
      toast.success('Modelos exportados exitosamente');
    } catch (error: any) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const blob = await AssetModelService.downloadTemplate();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'asset-models-template.xlsx';
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await AssetModelService.importFromExcel(file);
      ...
      if (result.data?.duplicados && result.data.duplicados.length > 0) {
        ...
        if (result.data.insertados > 0) {
          toast.success(`${result.data.insertados} modelos creados. Se detectaron ${result.data.duplicados.length} duplicados.`);
        } else {
          toast(`Se detectaron ${result.data.duplicados.length} modelos duplicados. Revisa el modal para decidir si actualizar.`);
        }
        ...
        toast.success(`${result.data.insertados} modelos creados exitosamente`);
      }
  ...
  const handleUpdateDuplicates = async () => {
    ...
    try {
      const result = await AssetModelService.updateDuplicatesFromExcel(duplicatesData);
      toast.success(result.message || `${result.data.actualizados} modelos actualizados`);
  ...
  const columns: Column<any>[] = [
    ...
    },
    {
      ...
      width: '12%',
      sortable: true,
      render: (model) => (model.creator?.userName || '-'),
    },
    {
      key: 'createdAt',
      label: 'Creación',
      sortable: true,
      render: (model) => (model.createdAt ? new Date(model.createdAt).toLocaleString('es-ES') : '-'),
    },
  ];
...
return (
    <div>
      ...
      <div className="card">
        {isLoading ? (
          <div className="text-center py-12">
            <p className="text-gray-600 dark:text-gray-400">Cargando modelos de activos...</p>
          </div>
        ) : (
          <DataTable
            data={data?.data || []}
            ...
            emptyMessage="No se encontraron modelos de activos"
            defaultPageSize={10}
          />
        )}
      </div>
      ...
      <DuplicatesModal
        ...
        title="Modelos Duplicados Detectados"
        message="Los siguientes modelos ya existen en la base de datos. ¿Deseas actualizarlos con los datos del Excel?"
        columns={[
          { key: 'fila', label: 'Fila' },
          { key: 'tipo', label: 'Tipo' },
