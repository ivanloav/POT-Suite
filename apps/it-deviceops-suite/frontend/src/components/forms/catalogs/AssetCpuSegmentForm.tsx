...
interface AssetCpuSegmentFormData {
  code: string;
  name: string;
  isActive: boolean;
}

interface AssetCpuSegmentFormProps {
  mode: 'create' | 'edit';
  segmentId?: number;
  onSuccess: () => void;
  onCancel: () => void;
...
export default function AssetCpuSegmentForm({ mode, segmentId, onSuccess, onCancel, onHeaderButtons }: AssetCpuSegmentFormProps) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['asset-cpu-segments', segmentId],
    queryFn: () => assetCpuSegmentService.getById(segmentId!),
    enabled: mode === 'edit' && !!segmentId,
  });
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<AssetCpuSegmentFormData>({
    defaultValues: {
      code: '',
      name: '',
      isActive: true,
  ...
  const createMutation = useMutation({
    mutationFn: assetCpuSegmentService.create,
    onSuccess: () => {
      toast.success('Segmento de CPU creado exitosamente');
      queryClient.invalidateQueries({ queryKey: ['asset-cpu-segments'] });
      onSuccess();
    },
    onError: (error: any) => {
      toast.error(error.response?.data?.message || 'Error al crear el segmento');
    },
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: AssetCpuSegmentFormData }) =>
      assetCpuSegmentService.update(id, data),
    onSuccess: () => {
      toast.success('Segmento de CPU actualizado exitosamente');
      queryClient.invalidateQueries({ queryKey: ['asset-cpu-segments'] });
      setIsEditing(false);
      onSuccess();
    },
    onError: (error: any) => {
      toast.error(error.response?.data?.message || 'Error al actualizar el segmento');
    },
  });

  const onSubmit = (formData: AssetCpuSegmentFormData) => {
    if (mode === 'create') {
      createMutation.mutate(formData);
    } else if (segmentId) {
      updateMutation.mutate({ id: segmentId, data: formData });
    }
  };
