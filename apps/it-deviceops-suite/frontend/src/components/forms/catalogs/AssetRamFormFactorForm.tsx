...
interface AssetRamFormFactorFormData {
  code: string;
  name: string;
  isActive: boolean;
}

interface AssetRamFormFactorFormProps {
  mode: 'create' | 'edit';
  formFactorId?: number;
  onSuccess: () => void;
  onCancel: () => void;
...
export default function AssetRamFormFactorForm({ mode, formFactorId, onSuccess, onCancel, onHeaderButtons }: AssetRamFormFactorFormProps) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['asset-ram-form-factors', formFactorId],
    queryFn: () => assetRamFormFactorService.getById(formFactorId!),
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<AssetRamFormFactorFormData>({
    defaultValues: {
      code: '',
      name: '',
      isActive: true,
  ...
  const createMutation = useMutation({
    mutationFn: assetRamFormFactorService.create,
    onSuccess: () => {
      toast.success('Form factor RAM creado exitosamente');
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: AssetRamFormFactorFormData }) =>
      assetRamFormFactorService.update(id, data),
    onSuccess: () => {
      toast.success('Form factor RAM actualizado exitosamente');
      queryClient.invalidateQueries({ queryKey: ['asset-ram-form-factors'] });
      setIsEditing(false);
      onSuccess();
    },
  ...
  const onSubmit = (formData: AssetRamFormFactorFormData) => {
    if (mode === 'create') {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div>
        ...
        <input
          ...
          disabled={mode === 'edit' && !isEditing}
          className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 disabled:bg-gray-100 dark:disabled:bg-gray-800 disabled:cursor-not-allowed"
          placeholder="Ej: SO-DIMM 260-pin"
        />
