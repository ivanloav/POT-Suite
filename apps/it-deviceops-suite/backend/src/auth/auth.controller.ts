...
export class AuthController {
  ...
  @Post('suite-login')
  @ApiOperation({ summary: 'Login Operations Hub', description: 'Inicia sesión usando cookie httpOnly del portal' })
  @ApiResponse({ status: 200, description: 'Login exitoso' })
  @ApiResponse({ status: 401, description: 'Token inválido' })
  async suiteLogin(@Request() req: any, @Res({ passthrough: true }) res: Response) {
    const suiteToken = req.cookies?.access_token;
    if (!suiteToken) {
      throw new UnauthorizedException('Token no proporcionado');
    }

    const suiteSecret = process.env.SUITE_JWT_SECRET || process.env.JWT_SECRET;
    if (!suiteSecret) {
    ...
    let payload: any;
    try {
      payload = verify(suiteToken, suiteSecret);
    } catch (error) {
    ...
    const clientIp = this.getClientIp(req);
    const userAgent = req.headers['user-agent'] || null;
    const result = await this.authService.loginWithEmail(email, clientIp, userAgent);
