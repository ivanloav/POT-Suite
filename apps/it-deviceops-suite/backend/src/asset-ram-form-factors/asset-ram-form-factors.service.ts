...
import { CreateRamFormFactorDto, UpdateRamFormFactorDto } from './dto/ram-form-factor.dto';
import * as XLSX from 'xlsx';

@Injectable()
export class AssetRamFormFactorsService {
  constructor(
    @InjectRepository(AssetRamFormFactor)
    private formFactorRepository: Repository<AssetRamFormFactor>,
  ) {}

  async getAll(): Promise<AssetRamFormFactor[]> {
  ...
  async getById(id: number): Promise<AssetRamFormFactor> {
    ...
    if (!formFactor) {
      throw new NotFoundException(`Form factor de RAM con ID ${id} no encontrado`);
    }

    return formFactor;
  }

  async create(dto: CreateRamFormFactorDto, userId: number): Promise<AssetRamFormFactor> {
    ...
    try {
      ...
      if (error.code === '23505') {
        throw new ConflictException(`Ya existe un form factor de RAM con el código "${dto.code}"`);
      }
      throw error;
    }
  }

  async update(id: number, dto: UpdateRamFormFactorDto, userId: number): Promise<AssetRamFormFactor> {
  ...
  async exportToExcel(): Promise<Buffer> {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Form Factors RAM');
  ...
  async generateTemplate(): Promise<Buffer> {
    const data = [
      {
        Código: 'DIMM',
        Nombre: 'DIMM (Dual In-line Memory Module)',
        Estado: 'Activo',
      },
      {
        Código: 'SO-DIMM',
        Nombre: 'SO-DIMM (Small Outline DIMM)',
        Estado: 'Activo',
      },
    ];
