...
@ApiTags('asset-ram-form-factors')
@ApiBearerAuth()
@Controller('asset-ram-form-factors')
@UseGuards(JwtAuthGuard)
export class AssetRamFormFactorsController {
  constructor(private readonly formFactorsService: AssetRamFormFactorsService) {}
  ...
  @Post()
  async create(@Body() dto: CreateRamFormFactorDto, @Req() req: any) {
    const data = await this.formFactorsService.create(dto, req.user.userId);
    return { success: true, data, message: 'Form factor de RAM creado exitosamente' };
  }
  ...
  async exportToExcel(@Res() res: Response) {
    ...
    res.set({
      ...
      'Content-Disposition': 'attachment; filename=form-factors-ram.xlsx',
  ...
  async downloadTemplate(@Res() res: Response) {
    ...
    res.set({
      'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
      'Content-Disposition': 'attachment; filename=plantilla-form-factors-ram.xlsx',
      'Content-Length': buffer.length,
    });
    res.send(buffer);
  }
  ...
  @Put(':id')
  async update(@Param('id') id: string, @Body() dto: UpdateRamFormFactorDto, @Req() req: any) {
    const data = await this.formFactorsService.update(+id, dto, req.user.userId);
    return { success: true, data, message: 'Form factor de RAM actualizado exitosamente' };
  }
}
