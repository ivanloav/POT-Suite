...
* Clave foránea compuesta: `(site_id, product_id)` → `products` con `ON DELETE RESTRICT`
...
| ID_PRODUCTO         | product_id            | BIGINT          | ID del producto. Ahora es clave foránea hacia `products`.                       |
...
* `products`: clave foránea compuesta `(site_id, product_id)` con `ON DELETE RESTRICT`
...
```sql
INSERT INTO order_items (site_id, order_id, line_number, product_id, product_ref, quantity, unit_price)
VALUES (1, 1001, 1, 2001, 'PRD001', 2, 19.95);
```
...
CREATE INDEX idx_order_items_site_productid ON order_items (site_id, product_id);
```
