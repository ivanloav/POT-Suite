...
import AssetBrandForm from '@/components/forms/catalogs/AssetBrandForm';
...
export default function AssetBrandsPage() {
  ...
  const canManageResource = canManage('catalogs') || canManage('assetBrands');

  const { data, isLoading } = useQuery({
    queryKey: ['asset-brands', selectedSiteId, filters],
    queryFn: () => assetBrandsService.getAll(),
  });
  ...
  const handleExport = async () => {
    try {
      const blob = await assetBrandsService.exportToExcel();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'asset-brands.xlsx';
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
      toast.success('Marcas exportadas exitosamente');
    } catch (error: any) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const blob = await assetBrandsService.downloadTemplate();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'asset-brands-template.xlsx';
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await assetBrandsService.importFromExcel(file);
      ...
      if (result.data?.duplicados && result.data.duplicados.length > 0) {
        ...
        if (result.data.insertados > 0) {
          toast.success(`${result.data.insertados} marcas creadas. Se detectaron ${result.data.duplicados.length} duplicados.`);
        } else {
          toast(`Se detectaron ${result.data.duplicados.length} marcas duplicadas. Revisa el modal para decidir si actualizar.`);
        }
        ...
        toast.success(`${result.data.insertados} marcas creadas exitosamente`);
      }
  ...
  const handleUpdateDuplicates = async () => {
    ...
    try {
      const result = await assetBrandsService.updateDuplicatesFromExcel(duplicatesData);
      toast.success(result.message || `${result.data.actualizados} marcas actualizadas`);
  ...
  const columns: Column<any>[] = [
    ...
    },
    {
      ...
      width: '15%',
      sortable: true,
      render: (brand) => (brand.creator?.userName || '-'),
    },
    {
      key: 'createdAt',
      ...
      width: '15%',
      sortable: true,
      render: (brand) => (brand.createdAt ? new Date(brand.createdAt).toLocaleString('es-ES') : '-'),
    },
  ];
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-gray-600 dark:text-gray-400">Cargando marcas...</div>
      </div>
    );
  }
    ...
    <div>
      ...
      {duplicatesData && (
        <DuplicatesModal
          ...
          title="Marcas Duplicadas Detectadas"
          message="Las siguientes marcas ya existen en la base de datos. ¿Deseas actualizarlas?"
        />
      )}
    </div>
  );
