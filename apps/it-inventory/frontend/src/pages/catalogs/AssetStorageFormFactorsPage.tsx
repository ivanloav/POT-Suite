...
import AssetStorageFormFactorForm from '@/components/forms/catalogs/AssetStorageFormFactorForm';
import toast from 'react-hot-toast';
import { ActionButton } from '@/components/shared/ActionButton';

export default function AssetStorageFormFactorsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['asset-storage-form-factors', selectedSiteId, filters],
  ...
  const handleExport = async () => {
    try {
      const blob = await assetStorageFormFactorService.exportToExcel();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'form-factors-disco.xlsx';
  ...
  const handleDownloadTemplate = async () => {
    try {
      const blob = await assetStorageFormFactorService.downloadTemplate();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = 'asset-storage-form-factors-template.xlsx';
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const result = await assetStorageFormFactorService.importFromExcel(file);
  ...
  const handleUpdateDuplicates = async () => {
    ...
    try {
      const result = await assetStorageFormFactorService.updateDuplicatesFromExcel(duplicatesData);
  ...
  return (
    <div>
      ...
      <div className="card">
        {isLoading ? (
          <div className="text-center py-12">
            <p className="text-gray-600 dark:text-gray-400">Cargando form factors de disco...</p>
          </div>
        ) : (
          <DataTable
            data={data || []}
            ...
            emptyMessage="No se encontraron form factors de disco"
            defaultPageSize={10}
          />
        )}
      </div>
      ...
      <Modal
        ...
        title="Crear Nuevo Form Factor de Disco"
        size="lg"
      >
        <AssetStorageFormFactorForm
          mode="create"
      ...
      <Modal
        ...
        title="Detalles del Form Factor de Disco"
        size="lg"
      ...
      >
        <AssetStorageFormFactorForm
          ...
          onSuccess={() => queryClient.invalidateQueries({ queryKey: ['asset-storage-form-factors'] })}
      ...
      <DuplicatesModal
        ...
        title="Duplicados de Form Factors de Disco"
