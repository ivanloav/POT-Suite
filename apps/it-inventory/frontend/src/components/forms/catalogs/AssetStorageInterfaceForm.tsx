...
interface AssetStorageInterfaceFormData {
  code: string;
  name: string;
  isActive: boolean;
}

interface AssetStorageInterfaceFormProps {
  mode: 'create' | 'edit';
  interfaceId?: number;
  onSuccess: () => void;
  onCancel: () => void;
...
export default function AssetStorageInterfaceForm({ mode, interfaceId, onSuccess, onCancel, onHeaderButtons }: AssetStorageInterfaceFormProps) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['asset-storage-interfaces', interfaceId],
    queryFn: () => assetStorageInterfaceService.getById(interfaceId!),
    enabled: mode === 'edit' && !!interfaceId,
  });
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<AssetStorageInterfaceFormData>({
    defaultValues: {
      code: '',
      name: '',
      isActive: true,
  ...
  const createMutation = useMutation({
    mutationFn: assetStorageInterfaceService.create,
    onSuccess: () => {
      toast.success('Interfaz de almacenamiento creada exitosamente');
      queryClient.invalidateQueries({ queryKey: ['asset-storage-interfaces'] });
      onSuccess();
    },
    onError: (error: any) => {
      toast.error(error.response?.data?.message || 'Error al crear la interfaz');
    },
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: AssetStorageInterfaceFormData }) =>
      assetStorageInterfaceService.update(id, data),
    onSuccess: () => {
      toast.success('Interfaz de almacenamiento actualizada exitosamente');
      queryClient.invalidateQueries({ queryKey: ['asset-storage-interfaces'] });
      setIsEditing(false);
      onSuccess();
    },
    onError: (error: any) => {
      toast.error(error.response?.data?.message || 'Error al actualizar la interfaz');
    },
  });

  const onSubmit = (formData: AssetStorageInterfaceFormData) => {
    if (mode === 'create') {
      createMutation.mutate(formData);
    } else if (interfaceId) {
      updateMutation.mutate({ id: interfaceId, data: formData });
    }
  };
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div>
        ...
        <input
          ...
          disabled={mode === 'edit' && !isEditing}
          className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 disabled:bg-gray-100 dark:disabled:bg-gray-800 disabled:cursor-not-allowed"
          placeholder="Ej: SATA III (6 Gb/s)"
        />
