...
interface AssetRamMemoryTypeFormData {
  code: string;
  name: string;
  isActive: boolean;
}

interface AssetRamMemoryTypeFormProps {
  mode: 'create' | 'edit';
  memoryTypeId?: number;
  onSuccess: () => void;
  onCancel: () => void;
...
export default function AssetRamMemoryTypeForm({ mode, memoryTypeId, onSuccess, onCancel, onHeaderButtons }: AssetRamMemoryTypeFormProps) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['asset-ram-memory-types', memoryTypeId],
    queryFn: () => assetRamMemoryTypeService.getById(memoryTypeId!),
    enabled: mode === 'edit' && !!memoryTypeId,
  });
  ...
  const {
    ...
    formState: { errors },
    reset,
  } = useForm<AssetRamMemoryTypeFormData>({
    defaultValues: {
      code: '',
      name: '',
      isActive: true,
  ...
  const createMutation = useMutation({
    mutationFn: assetRamMemoryTypeService.create,
    onSuccess: () => {
      toast.success('Tipo de memoria RAM creado exitosamente');
      queryClient.invalidateQueries({ queryKey: ['asset-ram-memory-types'] });
      onSuccess();
    },
    onError: (error: any) => {
      toast.error(error.response?.data?.message || 'Error al crear el tipo de memoria');
    },
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: AssetRamMemoryTypeFormData }) =>
      assetRamMemoryTypeService.update(id, data),
    onSuccess: () => {
      toast.success('Tipo de memoria RAM actualizado exitosamente');
      queryClient.invalidateQueries({ queryKey: ['asset-ram-memory-types'] });
      setIsEditing(false);
      onSuccess();
    },
    onError: (error: any) => {
      toast.error(error.response?.data?.message || 'Error al actualizar el tipo de memoria');
    },
  });

  const onSubmit = (formData: AssetRamMemoryTypeFormData) => {
    if (mode === 'create') {
      createMutation.mutate(formData);
    } else if (memoryTypeId) {
      updateMutation.mutate({ id: memoryTypeId, data: formData });
    }
  };
