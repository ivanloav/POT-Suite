...
interface AssetCpuVendorFormData {
  code: string;
  name: string;
  isActive: boolean;
}

interface AssetCpuVendorFormProps {
  mode: 'create' | 'edit';
  vendorId?: number;
  onSuccess: () => void;
  onCancel: () => void;
...
export default function AssetCpuVendorForm({ mode, vendorId, onSuccess, onCancel, onHeaderButtons }: AssetCpuVendorFormProps) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['asset-cpu-vendors', vendorId],
    queryFn: () => assetCpuVendorService.getById(vendorId!),
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<AssetCpuVendorFormData>({
    defaultValues: {
      code: '',
      name: '',
      isActive: true,
  ...
  const createMutation = useMutation({
    mutationFn: assetCpuVendorService.create,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['asset-cpu-vendors'] });
      toast.success('Vendedor de CPU creado exitosamente');
      onSuccess();
    },
    onError: (error: any) => {
      toast.error(error.response?.data?.message || 'Error al crear el vendedor');
    },
  });

  const updateMutation = useMutation({
    mutationFn: (formData: AssetCpuVendorFormData) => assetCpuVendorService.update(vendorId!, formData),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['asset-cpu-vendors', vendorId] });
      queryClient.invalidateQueries({ queryKey: ['asset-cpu-vendors'] });
      toast.success('Vendedor de CPU actualizado exitosamente');
      setIsEditing(false);
      onSuccess();
    },
    onError: (error: any) => {
      toast.error(error.response?.data?.message || 'Error al actualizar el vendedor');
    },
  });

  const onSubmit = (formData: AssetCpuVendorFormData) => {
    if (mode === 'create') {
  ...
  return (
    <form id="cpu-vendor-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      {/* Code */}
      <div>
