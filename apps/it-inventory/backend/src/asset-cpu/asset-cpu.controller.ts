...
import { CreateAssetCpuDto, UpdateAssetCpuDto } from './dto/asset-cpu.dto';
import { JwtAuthGuard } from '../middleware/jwt-auth.guard';
import { RequestWithUser } from '../common/interfaces/request-with-user.interface';

@ApiTags('asset-cpus')
@Controller('asset-cpu')
@ApiBearerAuth()
export class AssetCpuController {
  constructor(private readonly assetCpuService: AssetCpuService) {}
  ...
  async getAssetModels(
    ...
    const data = await this.assetCpuService.getAssetCpus(
  ...
  @Get('vendors')
  @ApiOperation({ summary: 'Listar vendors de CPU' })
  @ApiResponse({ status: 200, description: 'Lista de vendors' })
  async getVendors() {
    const data = await this.assetCpuService.getCpuVendors();
    return { data };
  }

  @Get('segments')
  @ApiOperation({ summary: 'Listar segmentos de CPU' })
  @ApiResponse({ status: 200, description: 'Lista de segmentos' })
  async getSegments() {
    const data = await this.assetCpuService.getCpuSegments();
    return { data };
  }
  ...
  async exportToExcel(@Res() res: Response) {
    const buffer = await this.assetCpuService.exportToExcel();
    res.set({
      'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
      'Content-Disposition': 'attachment; filename=asset-cpus.xlsx',
    });
    res.send(buffer);
  }
  ...
  async downloadTemplate(@Res() res: Response) {
    const buffer = await this.assetCpuService.generateTemplate();
    res.set({
      'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
      'Content-Disposition': 'attachment; filename=asset-cpus-template.xlsx',
    });
    res.send(buffer);
  }
  ...
  async createAssetCpu(@Body() dto: CreateAssetCpuDto, @Request() req: RequestWithUser) {
    const userId = req.user.userId;
    const data = { ...dto, createdBy: userId };
    const result = await this.assetCpuService.createAssetCpu(data);
    return {
  ...
  async getAssetCpuById(@Param('id') id: string) {
    const data = await this.assetCpuService.getAssetCpuById(parseInt(id));
    return { data };
  }
  ...
  async updateAssetCpu(@Param('id') id: string, @Body() dto: UpdateAssetCpuDto, @Request() req: RequestWithUser) {
    const userId = req.user.userId;
    const data = { ...dto, updatedBy: userId };
    const result = await this.assetCpuService.updateAssetCpu(parseInt(id), data);
    return {
  ...
  async importFromExcel(
    ...
    const userId = req.user.userId;
    const result = await this.assetCpuService.importFromExcel(file.buffer, userId);
  ...
  async updateDuplicatesFromExcel(@Body() body: { duplicates: any[] }) {
    const result = await this.assetCpuService.updateDuplicatesFromExcel(body.duplicates);
    return {
